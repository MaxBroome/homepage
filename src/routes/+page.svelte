<script lang="ts">
  ...
  const getAge = () =>
    (Date.now() - new Date(DATA.birthday).getTime()) / 31557600000;
  ...
  function openHatsModal() {
    isModalOpen = true;
    posthog.capture("hats_modal_opened");
  }
</script>
      ...
      <div class="flex justify-between gap-2">
        <div class="flex flex-1 flex-col space-y-1.5">
          ...
          <BlurFade delay={BLUR_FADE_DELAY * 1.2}>
            <div
              class="flex items-center gap-4 pt-2 text-sm text-muted-foreground"
            >
      ...
      >
        {@html renderMarkdown(
          DATA.summary.replace(
            ...
            'I wear a lot of <button id="hats-trigger" class="underline decoration-dotted hover:text-foreground transition-colors cursor-pointer">hats</button>',
          ),
        )}
      </div>
    </BlurFade>
      ...
      <BlurFade delay={BLUR_FADE_DELAY}>
        ...
        >
          <div class="space-y-2">
            ...
            >
              I&apos;ve worked on a variety of projects, from simple scripts to
              complex SaaS applications. Here are a few of my favorites.
            </p>
          </div>
        </div>
      </BlurFade>

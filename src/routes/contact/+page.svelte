		...
		<div class="mx-auto w-full max-w-2xl space-y-8 pt-12">
			<BlurFade delay={BLUR_FADE_DELAY}>
				<div class="space-y-3 text-center">
					<h1 class="text-3xl font-bold tracking-tight sm:text-5xl">Get in Touch</h1>
			...
			<BlurFade delay={BLUR_FADE_DELAY * 2}>
				<div class="diagram-wrapper relative">
					<div class="diagram-grid">
						...
						<button on:click={copyEmail} class="email-link {copied ? 'email-copied' : ''}" style="grid-column: 1 / 4;">
							<span class="email-username">{username}</span><span class="email-at">@</span><span class="email-domain">{domain}</span>
						</button>
					...
					<button on:click={copyEmail} class="absolute left-0 top-1/2 -translate-x-12 -translate-y-1/2 p-2 text-muted-foreground hover:text-foreground transition-colors">
						{#if copied}
							<Check class="h-5 w-5" />
						{:else}
							<Copy class="h-5 w-5" />
			...
			<BlurFade delay={BLUR_FADE_DELAY * 3}>
				<div class="text-center space-y-4">
					<p class="text-sm text-muted-foreground">
						<span class="text-muted-foreground/50">{'// '}</span>I'll respond whenever I can. Looking forward to hearing from you!
					</p>
					<p class="text-xs text-muted-foreground/70">
						<span class="text-muted-foreground/40">{'// '}</span>Why is this its own page? Well, because I don't want bots or AI scraping this page to get my email.
						<br>
						You are human, right?
					</p>
				</div>

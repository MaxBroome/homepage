<script lang="ts">
  ...
  function handleNavClick(event: MouseEvent, href: string, label: string) {
    posthog.capture("nav_item_clicked", { label, href });
    if (href.startsWith("#")) {
  ...
  function handleSocialClick(name: string, url: string) {
    posthog.capture("social_link_clicked", { name, url });
  }
        ...
        <Tooltip.Root openDelay={300}>
          <Tooltip.Trigger>
            <Button
              ...
              on:click={(e) => handleNavClick(e, item.href, item.label)}
            >
        ...
        <Tooltip.Root openDelay={300}>
          <Tooltip.Trigger>
            <Button
              ...
              rel={isExternalLink(social.url)
                ? "noopener noreferrer"
                : undefined}
              on:click={() => handleSocialClick(social.name, social.url)}
            >
